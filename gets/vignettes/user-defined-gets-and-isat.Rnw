...
\begin{center}
	...
	\footnote{
	This vignette is a revised and updated version of the article ``User-Specified General-to-Specific and Indicator Saturation Methods'', which was published in \href{https://journal.r-project.org}{The R Journal} (2020) 12:2, pages 388-401: \url{https://journal.r-project.org/archive/2021/RJ-2021-024/}. I am grateful to the Editor Catherine Hurley, an anonymous reviewer, Damian Clarke, {\'E}ric Dubois, Jonas Kurle, Felix Pretis, James Reade, Moritz Schwarz, Gareth Thomas, participants at the UseR! 2019 conference (Toulouse, July), the Statistics Norway seminar (May, 2018) and the Norges Bank seminar (April, 2018) for their helpful comments, suggestions and questions.		
	}
...
\subsection{The \code{getsFun()} function}
...
\subsection{The \code{blocksFun()} function}

The \code{blocksFun()} function was added to the package \textbf{gets} in version 0.24 (July 2020). It enables user-specified block-based GETS modelling, and its main usefulness is that it can handle more variables than observations $n$. Currently, the function \code{blocksFun()} accepts up to 31 arguments, most of which are the same as those of \code{getsFun()} (type \code{?blocksFun} for the details). In particular, the main principles outlined above for \code{getsFun()} also apply to \code{blocksFun()}.

The following code illustrates the most basic usage of \code{blocksFun()} with the default estimator, \code{ols()}, the same default as for \code{getsFun()}, in a situation where the number of regressors is larger than the number of observations:
%
\begin{verbatim}
  n <- 40 #number of observations
  k <- 60 #number of Xs
...
The package \textbf{gets} provides the generic function \code{gets()}. This enables the creation of GETS methods (S3) for models of arbitrary classes (type \code{?S3Methods} for more info on S3 methods). Here, this is illustrated for models of class \code{"lm"}. Since version 0.28, the \textbf{gets} package provides an method \code{gets()} for models of class \code{"lm"} (type \code{?gets.lm} for the details). So the illustration here is purely for illustration purposes.
...
\bibitem[\protect\citeauthoryear{Sucarrat, Kurle, Pretis, Reade, and
	Schwarz}{Sucarrat et~al.}{2020}]{SucarratKurlePretisReadeSchwarz2021getsV028}
Sucarrat, G., J.~Kurle, F.~Pretis, J.~Reade, and M.~Schwarz (2020).
\newblock {\em {gets}: {G}eneral-to-{S}pecific ({GETS}) {M}odelling and
	{I}ndicator {S}aturation ({ISAT}) {M}ethods}.
\newblock R package version 0.25.
