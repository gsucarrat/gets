...
\title{An introduction to the \textit{gets} package\thanks{Date: 6 August 2021. This paper is almost an exact reproduction of the paper published in Journal of Statistical Software: \url{https://doi.org/10.18637/jss.v086.i03}. The most important changes were made to accommodate the release of version 0.28, in which the default of the {\tt arx()} function argument {\tt mc} changed to {\tt TRUE}.}}
...
\begin{CodeChunk}
\begin{CodeInput}
R> mod02 <- arx(y, ar = 1:2, mxreg = mX, vcov.type = "white")
\end{CodeInput}
\end{CodeChunk}
%
...
\begin{CodeChunk}
\begin{CodeInput}
R> mod03 <- arx(eps, mc = FALSE, arch = 1)
\end{CodeInput}
\end{CodeChunk}
%
...
\begin{CodeChunk}
\begin{CodeInput}
R> mod04 <- arx(eps, mc = FALSE, arch = 1:3, asym = 2, vxreg = log(mX^2))
\end{CodeInput}
\end{CodeChunk}
%
...
\begin{CodeChunk}
\begin{CodeInput}
	R mod05 <- arx(yy, ar = 1:2, mxreg = mX, arch = 1:3, asym = 2,
...
\begin{CodeChunk}
\begin{CodeInput}
R> inflMod01 <- arx(inflData[, "infl"], ar = 1:4, mxreg = inflData[, 2:4],
+    vcov.type = "white")
\end{CodeInput}
\end{CodeChunk}
%
...
\begin{CodeChunk}
\begin{CodeInput}
R> inflMod02 <- arx(inflData[, "infl"], ar = 1:4, mxreg = inflData[, 2:4],
...
+    order.by = sp500Index)
R> sp500Mod01 <- arx(sp500Ret, mc = FALSE, arch = 1:5, log.ewma = c(5, 20, 60, 120),
+    asym = 1,
...
\begin{CodeChunk}
\begin{CodeInput}
R> info.criterion(as.numeric(logLik(sp500Mod01)), n = 8120, k = 17)
...
\begin{CodeChunk}
\begin{CodeInput}
R> mod06 <- arx(residuals(getsm05), mc = FALSE, arch = 1:3, asym = 2, vxreg = log(mX^2))
...
\begin{CodeChunk}
\begin{CodeInput}
R> inflMod04 <- arx(residuals(inflMod03), mc = FALSE, arch = 1:4,
+    vxreg = inflData[, 2:4])
...
\begin{CodeChunk}
\begin{CodeInput}
R> inflMod06 <- inflMod06 <- arx(inflData[, "infl"], ar = c(1, 4),
...
\begin{CodeChunk}
\begin{CodeInput}
R> info.criterion(as.numeric(logLik(sp500Mod03)), n = 8120, k = 7)
\end{CodeInput}
\end{CodeChunk}
%
