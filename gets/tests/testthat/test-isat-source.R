...
test_that("TEST MAIN isat() ARGUMENTS - additional test of predict.isat",{
  ...
  expect_snapshot_plot("Test_1",predict(isatmod, quiet = TRUE, newmxreg = matrix(0,12,5),  ci.levels=seq(0.20,0.95,by=0.05),n.sim=20000))
  expect_snapshot_plot("Test_2",predict(isatmod, quiet = TRUE, newmxreg = matrix(0,12,5),  ci.levels=seq(0.20,0.95,by=0.05),n.sim=20000, plot.options=list(shades=seq(20,95,by=5))))
  expect_snapshot_plot("Test_3",predict(isatmod, quiet = TRUE, newmxreg = matrix(0,12,5),  ci.levels=seq(0.20,0.95,by=0.05),n.sim=20000, plot.options=list(shades=seq(95,20,by=-5))))
  expect_snapshot_plot("Test_4",predict(isatmod, quiet = TRUE, newmxreg = matrix(0,12,5),  ci.levels=seq(0.20,0.95,by=0.05),n.sim=20000, plot.options=list(shades=seq(100,25,by=-5))))
  expect_snapshot_plot("Test_5",predict(isatmod, quiet = TRUE, newmxreg = matrix(0,12,5),  plot.options = list(keep=1)))
  expect_snapshot_plot("Test_6",predict(isatmod, quiet = TRUE, newmxreg = matrix(0,12,5),  plot.options = list(line.at.origin=FALSE)))
  expect_snapshot_plot("Test_7",predict(isatmod, quiet = TRUE, newmxreg = matrix(0,12,5),  plot.options = list(start.at.origin=TRUE)))
  expect_snapshot_plot("Test_8",predict(isatmod, quiet = TRUE, newmxreg = matrix(0,12,5),  plot.options = list(start.at.origin=TRUE, fitted=FALSE)))
  expect_snapshot_plot("Test_9",predict(isatmod, quiet = TRUE, newmxreg = matrix(0,12,5),  plot.options = list(dot.at.origin=FALSE)))
  expect_snapshot_plot("Test_10",predict(isatmod, quiet = TRUE, newmxreg = matrix(0,12,5), plot.options = list(hlines=c(-2,0,2,4,6,8))))
  expect_snapshot_plot("Test_11",predict(isatmod, quiet = TRUE, newmxreg = matrix(0,12,5), plot.options = list(col=c("darkred","green"))))
  expect_snapshot_plot("Test_12",predict(isatmod, quiet = TRUE, newmxreg = matrix(0,12,5), plot.options = list(lty=c(3,2))))
  expect_snapshot_plot("Test_13",predict(isatmod, quiet = TRUE, newmxreg = matrix(0,12,5), plot.options = list(lwd=3)))
  expect_snapshot_plot("Test_14",predict(isatmod, quiet = TRUE, newmxreg = matrix(0,12,5), plot.options = list(ylim=c(-8,16))))
  expect_snapshot_plot("Test_15",predict(isatmod, quiet = TRUE, newmxreg = matrix(0,12,5), plot.options = list(ylab="G-values")))
  expect_snapshot_plot("Test_16",predict(isatmod, quiet = TRUE, newmxreg = matrix(0,12,5), plot.options = list(main="Plot slightly lower when 'main' is specified")))
  expect_snapshot_plot("Test_17",predict(isatmod, quiet = TRUE, newmxreg = matrix(0,12,5), plot.options = list(legend.text=c("Prognose","Faktisk"))))
  expect_snapshot_plot("Test_18",predict(isatmod, quiet = TRUE, newmxreg = matrix(0,12,5), plot.options = list(fitted=FALSE)))
  expect_snapshot_plot("Test_19",predict(isatmod, quiet = TRUE, newmxreg=matrix(0,12,5),   plot.options = list(newmactual=rep(0,6))))
  expect_snapshot_plot("Test_20",predict(isatmod, quiet = TRUE, newmxreg=matrix(0,12,5),   plot.options = list(shades=c(95,50))))
  expect_snapshot_plot("Test_21",predict(isatmod, quiet = TRUE, newmxreg = matrix(0, 12, 5), plot.options = list(shades = c(50, 95)))) #invert shades
