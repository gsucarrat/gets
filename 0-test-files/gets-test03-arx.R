...
predict(mod01, n.ahead=5, newmxreg=matrix(0,5,NCOL(mX)),
  return=FALSE) #plot only
predict(mod01, n.ahead=5, newmxreg=matrix(0,5,NCOL(mX)),
  plot.options=list(fitted=TRUE))
predict(mod01, n.ahead=5, newmxreg=matrix(0,5,NCOL(mX)),
  plot.options=list(newmactual=rep(0,5)))
predict(mod01, n.ahead=5, newmxreg=matrix(0,5,NCOL(mX)),
  spec="variance") #should crash
predict(mod01, n.ahead=5, spec="variance") #should crash
recursive(mod01)
...
logLik(mod02)
plot(mod02)
predict(mod02) #should return the error-message 'newmxreg' is NULL
predict(mod02, n.ahead=5, newmxreg=matrix(0,5,NCOL(mX)))
predict(mod02, n.ahead=5, newmxreg=matrix(0,5,NCOL(mX)), plot=FALSE)
predict(mod02, spec="variance", n.ahead=5, newvxreg=matrix(0,5,NCOL(mX)))
predict(mod02, spec="variance", n.ahead=5, newvxreg=matrix(0,5,NCOL(mX)),
  plot.options=list(fitted=TRUE))
predict(mod02, spec="variance", n.ahead=5, newvxreg=matrix(0,5,NCOL(mX)),
  plot.options=list(newvactual=rep(1,5)))
predict(mod02, n.ahead=30, newmxreg=matrix(0,30,NCOL(mX)))
predict(mod02, n.ahead=5, newmxreg=matrix(0,5,NCOL(mX)),
  spec="mean")
predict(mod02, n.ahead=5, newmxreg=matrix(0,5,NCOL(mX)),
...
##test 1: used to yield error:
set.seed(123)
yy <- 2+rnorm(20)
mod04 <- arx(yy, mc=TRUE)
predict(mod04, n.ahead=1)

##test 2: used to yield error:
set.seed(123)
yy <- 25+rnorm(100)
y.actual <- 25+rnorm(12)
##create arx model
mod05 <- arx(yy,mc=TRUE,ar=1:2) ##y has mean 25
##generate some forecasts
predm05 <- predict(mod05,n.ahead=20, plot.options=list(keep=50))
predm05
##provide some actual values (needn't be same length as n.ahead)
predm05a <- predict(mod05, n.ahead=20,
...
set.seed(123)
yy <- rnorm(50)
mod06 <- arx(yy, mc=TRUE, ar=1)
predict(mod06, n.ahead=1) #used to be erroneous prediction
coef(mod06)[1] + coef(mod06)[2]*yy[length(yy)] #correct prediction
coef(mod06)[1] + coef(mod06)[2]*fitted(mod06)[length(fitted(mod06))] #reproducing old erroneous prediction

##test 4: same as test 3, but with X:
xx <- rnorm(length(yy))
mod07 <- arx(yy, mc=TRUE, ar=1, mxreg=xx)
predict(mod07, n.ahead=1, newmxreg=matrix(1,1,1)) #used to be erroneous prediction
coef(mod07)[1] + coef(mod07)[2]*yy[length(yy)] + coef(mod07)[3]*1 #correct prediction
coef(mod07)[1] + coef(mod07)[2]*fitted(mod07)[length(fitted(mod07))] + coef(mod07)[3]*1 #reproducing old erroneous prediction
...
##this does not work, should be fixed?:
...
##this works (compare with error above):
mod08 <- arx(y, ar=1:4, mxreg=mX, plot=FALSE)
predict(mod08, n.ahead=24, newmxreg=matrix(0,24,2))
